...
function cleanup($file, $highlight= false, $doReplace= true)
{
	if (checkFilePerms($file))
	{
		...
		$patterns= array(
						...
						//'if (...) then', 'if (...) {then}'
						//'else then', 'else {then}'
						array('~\s+$~', ''),// Match and remove any space on an empty line.
						array('~(\((?!\))|,|==|=(?!=|))(?! )~', '$1 '),
						array('~(?<! )((?<!\()\)|(?<!=)=)~', ' $1'),
		...
		$replacements= array();
		$contents= utf8_encode(file_get_contents($file));// The content to perform replacements on.
		$hfs= $highlight ? '[highlight]' : '';// Highlight Flag Start
		$hfe= $highlight ? '[/highlight]' : '';// Highlight Flag End

		if ($doReplace) $contents= str_replace($replaces[0], array("$hfs    $hfe", "$hfs\n$hfe", "$hfs\n$hfe"), $contents);
		else $contents= str_replace($replaces[0], array("$hfs\t$hfe", "$hfs\r\n$hfe", "$hfs \n$hfe"), $contents);
		// else $contents= str_replace($replaces[0], array("$hfs\t$hfe", "$hfs\r\n$hfe", "$hfs \n$hfe"), $contents);
		// else $contents= str_replace($replaces[0], explode(';;', $hfs.implode("$hfs;;$hfe", $replaces[1]).$hfe), $contents);

		foreach($patterns as $pattern)
		{
			if (!$doReplace && preg_match($pattern[0], $contents, $matches))
			{
				$contents= preg_replace($pattern[0], "$hfs\\0$hfe", $contents);
			}
			else $contents= preg_replace($pattern[0], $hfs.$pattern[1].$hfe, $contents);
		}

		if ($doReplace && count($replacements))
		{
			foreach ($replacements as $r) $contents= preg_replace('/\[highlight\].*?\[\/highlight\]/', "$hfs\\0$hfe", $contents);
		}
