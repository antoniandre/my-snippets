...
function cleanup($file, $highlight= false, $doReplace= true)
{
	...
	{
		$contentBefore= htmlentities(file_get_contents($file));// The content to perform replacements on.
		$contentPreviewBefore= '';
		$contentPreviewAfter= '';
		$contentAfter= '';
		...
		if ($doReplace)
		{
			//$contentBefore= str_replace(array('[highlight]', '[/highlight]'), '', $contentBefore);
			$contentBefore= str_replace($replaces[0], $replaces[1], $contentBefore);
		}
		else $contentBefore= str_replace($replaces[0], array("\t", "\r\n", " \n"), $contentBefore);
		// else $contentBefore= str_replace($replaces[0], array("\t", "\r\n", " \n"), $contentBefore);
		// else $contentBefore= str_replace($replaces[0], explode(';;', implode(";;", $replaces[1])), $contentBefore);

		foreach($patterns as $pattern)
		{
			if (!$doReplace && preg_match($pattern[0], $contentBefore, $matches))
			{
				$contentBefore= preg_replace($pattern[0], "\\0", $contentBefore);
			}
			else $contentBefore= preg_replace($pattern[0], $pattern[1], $contentBefore);
		}

		if ($doReplace && count($replacements))
		{
			foreach ($replacements as $r) $contentBefore= preg_replace('/\[highlight\].*?\[\/highlight\]/', "\\0", $contentBefore);
		}

		$contentBefore= str_replace(array('[highlight]', '[/highlight]'), array('<span class="highlight">', '</span>'), $contentBefore);
		if ($doReplace && count($replacements)) $contentBefore= preg_replace('/<span class="highlight">(.*?)<\/span> <span class="highlight">(.*?)<\/span>/', '<span class="highlight">$1$2</span>', $contentBefore);
